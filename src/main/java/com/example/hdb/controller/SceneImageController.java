...
public class SceneImageController extends BaseController {
    ...
    @Operation(summary = "일관성 있는 이미지 생성", description = "첫 번째 씬의 스타일을 유지하면서 다음 씬의 이미지를 생성합니다.")
    @PostMapping("/{projectId}/scenes/{sceneId}/images/generate-consistent")
    public ResponseEntity<ApiResponse<SceneImageResponse>> generateConsistentImage(
            @Parameter(description = "프로젝트 ID") @PathVariable Long projectId,
            @Parameter(description = "씬 ID") @PathVariable Long sceneId,
            @Valid @RequestBody com.example.hdb.dto.request.ConsistentImageGenerateRequest request,
            Authentication authentication) {

        log.info("=== Consistent Image Generation Started ===");
        ...
        try {
            ...
            SceneImageResponse response = sceneImageService.generateConsistentImage(
                    projectId,
                    sceneId,
            ...
            log.info("Consistent image generation completed successfully");
            return ResponseEntity.ok(ApiResponse.success("일관성 이미지 생성 성공", response));
